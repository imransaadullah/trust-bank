...
|---|---|---|
| **Identity & Tenant Mgmt** | Institutions (tenants), staff identity, RBAC/ABAC, service accounts, API key issuance | Who can do what, on whose behalf |
...
- API keys for the BaaS layer follow the **tiered-credential pattern you already designed for AuthCore** (`AUTHCORE_SCOPED_CLIENT_KEY_SPEC.md`): a scoped, allowlisted publishable-style key for anything a partner's client app might embed, and a secret key for server-to-server calls — same rationale applies here even more sharply, since a leaked banking API key is a much worse incident than a leaked auth key. Add app/origin binding and mandatory bot-protection on any client-embeddable tier, exactly as specified there.
...
1. **Phase 1 — Multi-tenant deposits/wallets, hardened.** Take the proven patterns from the existing `core-banking/` service (ledger, idempotency, outbox, reconciliation) and rebuild them greenfield with tenancy, HA, and the security architecture above from the start. This alone, done well, is sellable to a BaaS reseller or a small MFB.
2. **Phase 2 — BaaS/API platform layer on top of Phase 1.** Public API, tiered keys, sandbox, developer portal. Turns the internal core into a resellable product.
3. **Phase 3 — Loans & credit.** Highest-complexity domain (accrual, delinquency, provisioning, credit bureau integration) — sequence after the ledger and compliance plane are proven under real transaction volume.
4. **Phase 4 — Cards issuing/switching.** Deliberately last: the PCI-DSS scope and card-network integration have the longest external certification lead time, and benefit from a mature, already-audited ledger underneath them.
5. **Phase 5 — Deepen regulatory automation & treasury.** Full CBN/NDIC reporting automation, treasury/liquidity management — these matter most once you're carrying real institutional volume, and are easier to get right against a system that's already proven stable.
