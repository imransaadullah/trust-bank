...
- **Phase 2.5 — Identity, RBAC & Back-Office** *(gap — added August 2026; both bounded contexts were named in section 3 and required by section 2's "segregation of duties is architectural" principle from the start, but never scheduled into a phase — a planning gap, not a deliberate deferral)*. Slices 1 and 2 shipped: staff identity (password + mandatory MFA, no bypass path; short, sliding-expiry sessions; a fixed RBAC role set; branch/org-unit hierarchy built in from the start, not deferred until a multi-branch tenant asks for it — a `NATIONAL_MFB`'s branch structure is a basic expectation of the market segment, the same reframe already applied to BaaS/API and resilience posture) and maker-checker (dual approval on Compliance's case-review route — previously a free-text, unverified `reviewedBy` — and Ledger's manual-adjustment/reversal primitives; `requestedById != approvedById` enforced in code, not just by role). Section 7's "the engineer who deploys code cannot approve a manual ledger adjustment" is now true for these actions. Sequenced before Loans deliberately: loan approval (officer originates, credit manager approves) is the canonical maker-checker workflow, and this foundation now exists for it to build on. **Remaining**: Compliance's KYC/device/monitoring policy-publish routes aren't maker-checker-gated yet (they need `admin` scope, one tier above what's granted today — a bigger privilege for a first pass, and a lower-frequency action than a per-case or per-transaction one); Ledger's own `ledger_accounts` don't carry a `branch_id` tag yet, so branch modeling exists but isn't threaded through account-open/transaction flows; no staff-facing web UI.
