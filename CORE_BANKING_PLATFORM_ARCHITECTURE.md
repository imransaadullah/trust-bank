...
- **Phase 6 — Loose ends.** Not sequenced after Phase 5 the way 1 through 5 build on each other — a catch-all for real, already-named gaps that never justified a phase number of their own, collected here so nothing outstanding stays untracked. Pick items up opportunistically, not in order.
  ...
  - **Password reset / change-password flow** (`services/identity`) — a staff user with a lost or compromised password has no self-service recovery path today.
  ...
  - **mTLS for internal service-to-service calls** in a real hybrid deployment (section 9) — scoped credentials (`SERVICE_CREDENTIAL_MODEL.md`) shipped and make every caller revocable and tenant-bound; mTLS is the one hardening layer still queued, deliberately not built ahead of an actual hybrid customer needing it.
