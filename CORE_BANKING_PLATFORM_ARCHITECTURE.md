...
- **Phase 6 — Loose ends.** Not sequenced after Phase 5 the way 1 through 5 build on each other — a catch-all for real, already-named gaps that never justified a phase number of their own, collected here so nothing outstanding stays untracked. Pick items up opportunistically, not in order.
  ...
  - **Self-serve API key management and a usage-transparency dashboard** (Phase 2's fuller developer-portal vision, section 12) — **shipped** (August 2026). Re-reading the actual code rather than the stale doc found that issue/list/revoke (`services/gateway`'s `POST`/`GET /v1/tenants/:id/api-keys`, `.../revoke`) were already real and already self-serve — any tenant holding their own `admin`-tier key can already issue, list, and revoke `sandbox`/`production` keys for their own tenant with zero ops involvement; the only credential `provision-tenant.sh` ever hands out is the *first* `admin` key, the same root-credential pattern every serious API platform uses (an AWS root access key, Stripe's original secret key), not a missing bank-developer login system. This pass closed the two gaps that were real: a `rotate` endpoint (revoke + reissue in one call), and a genuine usage-transparency surface — `GET /v1/tenants/:id/usage` merges a new hourly rollup (`ApiKeyDailyUsage`, aggregated from the existing per-minute `RateLimitCounter`) with today's still-live counters, so a query is never a blind spot before the next tick. The rollup job also closed a latent, unrelated gap surfaced while designing this: `RateLimitCounter` had no retention at all — every window since the service started stayed a live row forever — now pruned automatically once rolled up. Per the Anchor-informed research already captured this engagement (the closest direct BaaS competitor runs negotiated subscription pricing, not self-serve metered billing), this is deliberately usage counts only — no cost, quota, or plan fields, and no billing/metering engine; that remains a real, separate initiative, not scoped here. Live-verified: rolled-up daily totals matched hand-inserted backdated counters exactly, a second immediate rollup rerun was a true no-op (the single-transaction insert-then-prune can't double-count on a crash/restart), a rotated key's old token got a real 401 while the new one passed straight through to real business logic, and the existing 429-after-limit rate-limiting behavior was confirmed unaffected by the new background job.
