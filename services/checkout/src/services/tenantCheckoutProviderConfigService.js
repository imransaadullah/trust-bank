// Which CheckoutProvider a tenant is on, and that provider's own
// credentials — mirrors services/cards' tenantCardProviderConfigService.js
// verbatim. Every checkout operation resolves its provider through here
// per call, not a module-level constant, since different tenants can be
...
async function setConfig({ tenantId, tenantSlug, provider, credentials }) {
  ...
  return prisma.tenantCheckoutProviderConfig.upsert({
    where: { tenantId },
...
async function getConfigByTenantId(tenantId) {
  const row = await prisma.tenantCheckoutProviderConfig.findUnique({ where: { tenantId } });
...
async function getConfigBySlug(tenantSlug) {
  const row = await prisma.tenantCheckoutProviderConfig.findUnique({ where: { tenantSlug } });
  if (!row || row.status !== 'active') {
