...
- Physical cards — fulfillment logistics are a different problem
  entirely. This slice is virtual-only.
- A real card-issuing-as-a-service integration (Marqeta, Galileo, or a
  Nigeria-specific program manager) — the no-op provider is the only one
  that will ever exist until a real commercial relationship does. A
  concrete provider is a future slice behind the same
  `CardIssuingProvider` contract; nothing else in this service changes
  when that day comes.
- Real card-network/processor webhook receiving — `authorize`/`settle`
  exist and work exactly the way a real processor's webhook eventually
  would call them, but nothing calls them automatically yet.
...
- Gateway proxying — Cards isn't part of the public API external banks
  integrate against yet. A small, mechanical follow-up once this core
  lifecycle is proven, matching how the Gateway itself was built as
  Phase 2 after Phase 1's core already existed.
...
prisma/                 Card (status/limits/tokenized providerRef, no PAN),
                         ...
                         daily-limit check), ApiCredential, TenantLedgerCredential
src/providers/
  cardIssuingProvider.js    abstract base — issueCard/freezeCard/unfreezeCard/closeCard
  noopCardIssuingProvider.js the only implementation today — logs, never calls out
src/services/
